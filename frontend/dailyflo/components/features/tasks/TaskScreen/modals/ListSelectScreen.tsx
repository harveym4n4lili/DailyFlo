...
export function ListSelectScreen() {
  ...
  const params = useLocalSearchParams<{ taskId?: string; habitId?: string }>();
  ...
  const isHabitMode = Boolean(habitIdForDraft);
  ...
  const handlePick = async (id: string | null) => {
    ...
    if (isHabitMode) {
      // TODO: PATCH habit listId when backend exposes the field on habits API
